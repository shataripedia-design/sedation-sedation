...
interface Drug {
  id: string;
  name: string;
  category: "sedation" | "analgesia" | "nmb" | "reversal";
  class: string;
  routes: string[];
  doses: Dose[];
  warnings: string[];
...
const WEANING: WeaningGuideline[] = [
  {
...
function calcDose(dose: Dose, weight: number): string | null {
...
type BadgeColor = "blue" | "green" | "purple" | "gray";
...
function ScaleCard({ scale }: { scale: AssessmentScale }) {
  const [open, setOpen] = useState(false);
  return (
...
function WeaningCard({ item }: { item: WeaningGuideline }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="rounded-xl border border-slate-700/60 bg-slate-800/40 overflow-hidden">
      <button onClick={() => setOpen((o) => !o)} className="w-full text-left p-4 flex items-start justify-between gap-3 hover:bg-white/5 transition-colors">
...
function ClinicalAlert({ tab }: { tab: string }) {
        ...
        {isSearchMode ? (
          <div className="space-y-3">
            <div className="text-xs text-slate-400 mb-3">
              Showing {allDrugs.length} result{allDrugs.length !== 1 ? "s" : ""} for &quot;{search}&quot;
            </div>
            {allDrugs.length === 0
      ...
      <footer className="max-w-3xl mx-auto px-4 py-6 mt-4 border-t border-slate-800">
        ...
        <div className="mt-4 pt-4 border-t border-slate-800/60 text-center">
          <p className="text-xs text-slate-500">
            ...
            <span className="text-sky-400 font-semibold">Dr. Mohammed Shatari</span>
          </p>
          <p className="text-xs text-slate-600 mt-0.5">
            🏥 King Saud Medical City · Pediatric Critical Care
          </p>
        </div>
      </footer>
